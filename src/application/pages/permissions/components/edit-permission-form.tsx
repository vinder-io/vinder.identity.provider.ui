...
interface EditPermissionFormProps {
  ...
  value: Permission;
  errors?: Partial<Record<keyof Permission, string>>;
  touched?: { name: boolean; description: boolean };
  loading?: boolean;
  onChange: (field: keyof Permission, value: string) => void;
  onBlur: (field: keyof Permission) => void;
  onSubmit: () => void;
}
...
export function EditPermissionForm({
  ...
  onBlur,
  onSubmit,
}: Readonly<EditPermissionFormProps>) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-sm w-full p-6">
        <form onSubmit={e => { e.preventDefault(); onSubmit(); }} className="space-y-6">
          <DialogHeader>
            <DialogTitle className="text-lg font-semibold">Edit permission</DialogTitle>
          </DialogHeader>
